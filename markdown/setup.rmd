<!-- This file is for LaTeX and R setups -->
<!-- Latex -->
<!-- Problem with Latex stuff is, that it won't compile to word correctly... -->

<!-- R -->

```{r setup-libraries}
# think about calling renv here to make sure everyone has the same packages installed
list.of.packages <- c("tidyverse", "rmarkdown", "papaja", "knitr", "acdcquery", "splithalf", "metafor")
new.packages <- list.of.packages[!(list.of.packages %in% installed.packages()[,"Package"])]
if(length(new.packages)) install.packages(new.packages)

library(papaja)
library(tidyverse)
library(rmarkdown)
library(knitr)
library(acdcquery)

r_refs("markdown/r-references.bib")

r_citations <- cite_r(
  file = "markdown/r-references.bib",
  pkgs = c("papaja", "tidyverse", "knitr", "acdcquery", "metafor", "lme4", "splithalf"),
  omit = FALSE,
  footnote = TRUE
  )

r_cite_intext <- cite_r(
  file = "markdown/r-references.bib",
  pkgs = c("papaja", "tidyverse", "knitr", "acdcquery", "metafor", "lme4", "splithalf"),
  omit = FALSE,
  footnote = FALSE
  )

```


```{r setup-print-percent}
print_percent <- function(percent){
  apa_msg = papaja::apa_num(percent)
  final = paste0(apa_msg, " %")
  return(final)
}

print_freq_percent <- function(freq){
  percent = freq*100
  apa_msg = papaja::apa_num(percent)
  final = paste0(apa_msg, " %")
  return(final)
}


print_rel <- function(rel){
  return(paste0("$\alpha~=~", papaja::apa_num(rel), "$"))
}

print_icc <- function(icc){
  return(paste0("$ICC~=~", papaja::apa_num(icc), "$"))
}
```

